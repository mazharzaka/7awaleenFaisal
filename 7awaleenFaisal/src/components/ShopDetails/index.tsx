              ...
              <div className="max-w-[539px] w-full">
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <button
                      onClick={() => {
                        const message = `السلام عليكم، أريد طلب:\n\n📦 ${product?.name}\nالسعر: ${product?.finalPrice.toLocaleString('ar-EG')} جنيه`;
                        const whatsappUrl = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '201000000000'}?text=${encodeURIComponent(message)}`;
                        window.open(whatsappUrl, '_blank');
                      }}
                      className="w-full sm:w-auto inline-flex items-center justify-center gap-2 font-medium text-white bg-[#25D366] py-3 px-7 rounded-md ease-out duration-200 hover:bg-[#1EAA52] hover:shadow-lg transition-all min-h-[44px]"
                    >
                      <svg
                        className="w-5 h-5"
                        fill="currentColor"
